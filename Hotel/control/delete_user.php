...
if (isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM `hotel`.`users` WHERE id = ?");
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            // ลบไม่สำเร็จ
            $_SESSION['message'] = "เกิดข้อผิดพลาดในการลบข้อมูล: " . htmlspecialchars($stmt->error);
        }

        $stmt->close();
    } else {
        $_SESSION['message'] = "ไม่สามารถเตรียมคำสั่ง SQL ได้: " . htmlspecialchars($conn->error);
    }
} else {
